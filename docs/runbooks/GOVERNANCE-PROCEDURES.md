...
"Data Owner" varies by domain - see ownership table in [GOVERNANCE.md § Ownership](../governance/GOVERNANCE.md).
...
These cover the failure modes called out in [ARCHITECTURE.md § Failure modes](../architecture/ARCHITECTURE.md). One canonical procedure per failure type. Updated after every incident review.
...
1. **Spec**: open a PR with a `/docs` markdown adding a row to the table in [DATA-MODEL.md § Gold](../architecture/DATA-MODEL.md): name, purpose, grain, columns, refresh cadence.
...
6. **Update lineage table** in [GOVERNANCE.md § Lineage](../governance/GOVERNANCE.md).
...
|---|---|---|
| "Show me the data flow for cost." | Manual lineage table | [GOVERNANCE.md § Lineage trace](../governance/GOVERNANCE.md) |
| "Show me the controls preventing bad data in billing." | GX suite + dbt tests + custom singular tests | [quality/](../../quality/), [dbt/](../../dbt/) |
| "Show me retention enforcement." | MinIO ILM rule + Kafka `retention.ms` + `VACUUM RETAIN` cadence | [GOVERNANCE.md § Lifecycle](../governance/GOVERNANCE.md) |
...
| "Where is the records of processing?" | [GOVERNANCE.md § GDPR](../governance/GOVERNANCE.md) + lineage trace + dbt docs | self-serve |
...
**See also**: [GOVERNANCE.md](../governance/GOVERNANCE.md) for the framework, [SECURITY.md](../governance/SECURITY.md) for the security-specific gaps, [STAKEHOLDERS.md § RACI](../stakeholders/STAKEHOLDERS.md) for the broader project RACI.
