...
> Companion to [GOVERNANCE.md](GOVERNANCE.md). That document describes **what** the governance framework is (quality framework, lineage, ownership, retention, compliance posture). This one describes **how to operate it** — the concrete procedures, RACIs, runbooks, and SLAs that translate the framework into day-to-day work.

If [GOVERNANCE.md](GOVERNANCE.md) answers "is data governed?", this document answers "what do I do on Tuesday morning when X happens?"
...
"Data Owner" varies by domain — see ownership table in [GOVERNANCE.md § Ownership](GOVERNANCE.md).
...
| Quality-gate failure resolution | Acknowledged < 2 h, root-caused < 8 h | Airflow task failure + Jira ticket; Slack/PagerDuty integration is planned | Data Eng lead | runbook §4.1 |
| Subject-erasure request | Completed within 30 calendar days | Deletion log + signoff | DPO + Data Eng lead | SOP §5.1 |
| Schema-breaking-change review | Reviewed within 2 working days of PR | GitHub PR review | Architect | §6 |
| Per-DAG runtime regression | Alert if > 1.5× rolling-30-day median | Airflow metric (planned: prometheus exporter) | Platform Eng | dashboard |
...
These cover the failure modes called out in [ARCHITECTURE.md § Failure modes](ARCHITECTURE.md). One canonical procedure per failure type. Updated after every incident review.
...
2. **Read [README.md § DuckDB trade-off](../README.md)** to confirm this is the known FFI issue.
...
5. **Long-term**: track upstream issue in `delta-kernel-rs`. If unresolved by Q4, escalate scale-out to Spark SQL (see Phase 4 roadmap in [PRESENTATION.md](PRESENTATION.md)).
...
1. **Spec**: open a PR with a `/docs` markdown adding a row to the table in [DATA-MODEL.md § Gold](DATA-MODEL.md): name, purpose, grain, columns, refresh cadence.
2. **Identify data class** per §7 (`pii`, `financial`, `internal`, `public`); add `meta.sensitivity` in dbt schema.yml.
3. **Build**:
...
6. **Update lineage table** in [GOVERNANCE.md § Lineage](GOVERNANCE.md).
...
2. **PR** to [quality/expectations/suite_silver_jobs.py](../quality/expectations/suite_silver_jobs.py):
   - Add the expectation
   - Choose `mostly` thresholding (rare to require 100%; usually 0.99–0.99)
...
3. **Backfill Silver**: run the existing Bronze→Silver job after restoring the needed Bronze data for the date range. In the current implementation this is an all-range rebuild rather than a parameterised date-window backfill.
...
- New `pii` columns require DPO approval (RACI §2).
...
|---|---|---|
| "Show me the data flow for cost." | Manual lineage table | [GOVERNANCE.md § Lineage trace](GOVERNANCE.md) |
| "Show me the controls preventing bad data in billing." | GX suite + dbt tests + custom singular tests | [quality/](../quality/), [dbt/](../dbt/) |
| "Show me retention enforcement." | MinIO ILM rule + Kafka `retention.ms` + `VACUUM RETAIN` cadence | [GOVERNANCE.md § Lifecycle](GOVERNANCE.md) |
...
| "How would you delete a user's data?" | SOP §5.1 | This document |
...
| "Who has access to financial data?" | RBAC config (today: defaults; production: SSO group ↔ API role mapping) | [SECURITY.md](SECURITY.md) |
| "Show me a Delta time-travel query." | `SELECT * FROM delta_scan('s3://gold/cost_attribution', version_as_of => 1)` | demonstrable live |
| "Where is the records of processing?" | [GOVERNANCE.md § GDPR](GOVERNANCE.md) + lineage trace + dbt docs | self-serve |
...
- [ ] Identify which Runbook (§4) you'd run if `batch_pipeline_daily` failed tomorrow
...
**See also**: [GOVERNANCE.md](GOVERNANCE.md) for the framework, [SECURITY.md](SECURITY.md) for the security-specific gaps, [STAKEHOLDERS.md § RACI](STAKEHOLDERS.md) for the broader project RACI.
