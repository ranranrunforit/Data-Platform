...
- Lifecycle: 90-day expiry on the `bronze/` bucket â€?Bronze is reproducible from generators / Kafka retention

### 2. Silver â€?schema-enforced + Great Expectations gate
...
| `expect_column_values_to_be_between(cost_usd, 0, â€?` mostly 0.99 | Catch billing bugs (allow NULL on in-flight jobs) |
...
| `expect_column_mean_to_be_between(cost_usd, 1.0, 500.0)` | Distribution drift detector â€?catches a mispriced GPU type |
...
### 3. Gold â€?dbt schema + custom tests
...
- **Source tests** ([dbt/models/staging/sources.yml](../../dbt/models/staging/sources.yml)): same checks at the Silver source â€?provides a second-line defence after GX.
...
Failure here does **not** roll back Gold (dbt-duckdb does not support transactional rollbacks across models). It marks the DAG run as failed and pages on-call. Gold remains queryable but flagged as suspect â€?see "Failure modes" in [architecture.md](../architecture/ARCHITECTURE.md).

---

## Lineage

The platform's lineage is end-to-end traceable through file paths and dbt's DAG. There is no central catalog (Datahub, Amundsen, Unity Catalog) â€?for a production deployment that would be the obvious next addition.
...
silver.jobs (source)
  â””â”€â”€ stg_jobs (view)
       â””â”€â”€ int_job_costs (table)
            â””â”€â”€ cost_attribution (table)
                 â””â”€â”€ tests: unique attribution_id, total_cost_usd >= 0,
                            assert_positive_costs

silver.inference (source)
  â””â”€â”€ stg_inference (view)
       â””â”€â”€ job_performance_sla (table)

silver.node_metrics (source)
  â””â”€â”€ gpu_utilization_hourly (table)
```
...
This is the platform's audit trail â€?every change to a Gold table is recoverable for 7 days.
...
| Gold Delta | Unlimited (for this project) | No vacuum on Gold â€?analysts may need historical versions |
...
In production, the same model would map to real org and user IDs from the IAM system. The cost attribution path (org â†?user â†?model) is the only place where an internal user could be re-identified â€?those tables would be access-controlled to finance + customer success only.
...
|---|---|---|---|
| Art. 5 (purpose limitation) | Data collected for one purpose may not be silently re-used | Each Gold mart has a documented purpose ([DATA-MODEL.md](../architecture/DATA-MODEL.md)); column-level descriptions in `dbt/models/marts/schema.yml` | Add a data-classification tag (`pii`, `internal`, `public`) to dbt model meta + enforce via `dbt-checkpoint` |
...
| Art. 17 (right to erasure) | Subject can request deletion within 30 days | Not implemented | Add a `subject_deletion_requests` queue â†?daily Spark job runs `DELETE FROM silver.jobs WHERE user_id IN (...)` + cascading Gold rebuild. Delta supports `DELETE` on Silver; Gold marts must be rebuilt from new Silver snapshot |
...
| Art. 32 (security of processing) | Pseudonymisation, encryption, integrity, availability | Delta ACID guarantees integrity; nightly `OPTIMIZE` + `VACUUM`; quality gate prevents bad data | TLS end-to-end (S3A, Kafka, Postgres); encryption at rest; MFA on admin access â€?see [SECURITY.md](SECURITY.md) |
...
|---|---|---|
| CC2.1 â€?Information & communication | This README + the `docs/` directory; ADRs document decisions | Onboarding runbooks; on-call rotation docs |
| CC5.1 â€?Logical access | Default `admin/admin` on Airflow, default MinIO root creds | OIDC / SAML SSO on every UI; per-org IAM roles; MFA mandatory for admin |
| CC6.1 â€?Logical access provisioning | None | Just-in-time access via SSO group â†?IAM role mapping |
| CC6.6 â€?Encryption | None at rest, none in transit on the bridge network | SSE-KMS on S3 buckets; TLS on every inter-service connection; managed certs via cert-manager / ACM |
| CC6.7 â€?Transmission of data | Plaintext within Docker | TLS 1.2+ end-to-end |
| CC6.8 â€?Vulnerability management | Pinned image versions in `Dockerfile.airflow`, `Dockerfile.spark`, `serving/Dockerfile`; pinned tool versions in CI | Trivy / Grype image scans in CI; Dependabot for Python deps; signed images via Sigstore |
| CC7.1 â€?Detection of system anomalies | `streaming_health_check` DAG monitors Kafka lag and Delta freshness | Centralised log aggregation (CloudWatch / ELK / Datadog); anomaly detection on metrics |
| CC7.2 â€?Incident response | Manual today | Runbook per failure mode in [architecture.md Â§ Failure modes](../architecture/ARCHITECTURE.md); on-call rotation |

#### Availability (A1)

| Control | Today | Production gap |
|---|---|---|
| A1.2 â€?System recovery | Spark task retries (2Ã— with 5 min delay); Airflow DAG has `max_active_runs=1` to prevent overlap; Kafka RF=3, min.isr=2 â€?single-broker failure causes no data loss | Multi-AZ deployment; managed RDS with automated backups + PITR; cross-region S3 replication |
| A1.3 â€?Backup procedures | Delta time-travel (7 days); Kafka 7-day retention; Bronze 90-day retention | Off-site backups of Postgres metadata + Gold tables; quarterly recovery tests |
...
| Control | Today |
|---|---|
| PI1.1 â€?Quality | Three-checkpoint quality framework (schema enforcement at Silver write, GX gate before Gold, dbt tests after Gold) |
| PI1.2 â€?Inputs validated | Spark `StructType` enforces input schema; Kafka producers serialise via a known schema |
| PI1.3 â€?Processing complete + accurate | `MERGE` for late-arriving events guarantees idempotency; Delta ACID guarantees no partial writes; pinned tool versions; deterministic seeds in tests |
| PI1.5 â€?Outputs reviewed | dbt schema + custom tests run after Gold; Custom test `assert_positive_costs.sql` blocks negative billing |

#### Confidentiality (C1)

| Control | Today | Production gap |
|---|---|---|
| C1.1 â€?Identification of confidential data | None | Tag dbt models with `meta: { sensitivity: confidential }` |
| C1.2 â€?Disposal | Delta `VACUUM`; MinIO ILM | Cryptographic erasure via per-tenant KMS keys |
...
|---|---|---|
| P1.1 â€?Notice | Synthetic data; no notice required | Privacy notice published; cookie consent on any UI handling user data |
| P3.1 â€?Choice and consent | n/a | Consent capture for data collection beyond contractual purpose |
| P4.1 â€?Collection minimisation | Generators emit only fields the marts use | Annual review of every Bronze schema for fields that are no longer needed |

### What this project demonstrates today

The platform is not SOC 2 / GDPR compliant today â€?it is a development-grade build. What it **does** demonstrate is the structural foundation those programmes require:

- **Documented data flow** â€?every column has a known source ([DATA-MODEL.md](../architecture/DATA-MODEL.md)) and every transformation has source code in version control.
- **Quality-gated promotion** â€?bad data is provably blocked from reaching analytical surfaces (GX gate + dbt tests + custom singular tests).
- **Lineage** â€?manual trace + dbt DAG + Delta time-travel. No table appears without a documented derivation path.
- **Idempotent re-runs** â€?re-running the pipeline does not produce different results given the same inputs (deterministic dedup, MERGE guarantees, surrogate keys).
- **Retention policies** â€?explicit at every layer (Bronze 90 d, Kafka 7 d, Silver Delta history 7 d).
- **Documented gaps** â€?every missing control is written down rather than implicit, so a compliance team can scope the gap exactly.

The "production gap" columns above are the explicit roadmap from development-grade to compliance-ready. Most rows resolve to existing tooling (SSO, KMS, TLS, Trivy, External Secrets Operator) wired into the K8s deployment from [ADR-005](../architecture-decisions/adr/005-docker-compose-dev-k8s-prod.md), not to additional architecture decisions.
