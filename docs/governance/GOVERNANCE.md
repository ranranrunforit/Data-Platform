...
Failure here does **not** roll back Gold (dbt-duckdb does not support transactional rollbacks across models). It marks the DAG run as failed and pages on-call. Gold remains queryable but flagged as suspect - see "Failure modes" in [architecture.md](../architecture/ARCHITECTURE.md).
...
| CC7.2 - Incident response | Manual today | Runbook per failure mode in [architecture.md § Failure modes](../architecture/ARCHITECTURE.md); on-call rotation |
...
- **Documented data flow** - every column has a known source ([DATA-MODEL.md](../architecture/DATA-MODEL.md)) and every transformation has source code in version control.
