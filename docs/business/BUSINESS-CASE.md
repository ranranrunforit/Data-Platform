...
> Why this platform exists, what pain it solves, what it costs, and when it pays back. Read alongside [STAKEHOLDERS.md](../stakeholders/STAKEHOLDERS.md) (who) and [COST-MODEL.md](COST-MODEL.md) (the unit economics).
...
Each pain is anchored to a concrete, recent, **measurable** event â€?not a hypothetical.

### Pain 1 â€?Billing reconciliation error costs real money
...
- No drill-down â€?finance can't trust a number they can't trace to source events

**Annualised cost of inaction**:
- Direct revenue leakage: $1.2M Ã— 4 quarters â‰?**$4.8M / year** at recent miss rate
- Even if the average miss rate is half that: **~$2.4M / year**

**How this platform fixes it**:
- `cost_attribution` mart materialised daily with `attribution_id = MD5(grain)` â€?drill-down is one query
- GX `mean(cost_usd) âˆ?[$1, $500]` detects pricing bugs within 24 h of Silver write
- `assert_positive_costs.sql` blocks negative billing
- 5-day close â†?1-day close â†?continuous after Phase 3

---

### Pain 2 â€?Data scientists blocked behind a ticket queue
...
**Why it happens**:
- No self-serve query surface â€?every cut requires a custom join across Postgres + S3 + InfluxDB
- No shared mart â€?every analyst re-derives "GPU spend by org by week" their own way
- DE team is the bottleneck, not because they're slow but because every request is a fresh problem

**Annualised cost of inaction**:
- 12 DS Ã— 8 h / week waiting Ã— $250 / fully-loaded hour Ã— 50 weeks = **$1.2M / year** in idle DS time
- Plus 4 DE FTEs Ã— 60% of time on bespoke pulls Ã— $200K loaded cost = **$0.48M / year** in DE opportunity cost
- **Subtotal: ~$1.7M / year**

**How this platform fixes it**:
- 8 REST endpoints cover the top-5 questions DS / Finance / CS ask
- Time-to-first-insight: 4 days â†?5 minutes
- DE team freed for platform work (new marts, quality rules) instead of bespoke pulls

---

### Pain 3 â€?SLO breaches reported by customers, not the platform
...
- Customer Success has no proactive alerting â€?they react to escalations

**Annualised cost of inaction**:
- One renewal discount of 12% on a $4M ARR customer â‰?**$0.48M lost ARR / year**
...
- Breach detection: 6 h â†?< 5 min (Phase 2) â†?< 30 s (Phase 3)

---

### Pain 4 â€?Compliance posture blocks new markets
...
**Why it happens**:
- No central lineage â€?can't answer "where does this number come from?" without a manual investigation
- No automated quality gate â€?auditor would have to take TechCorp's word on data integrity
- No documented retention â€?bronze data sits indefinitely in S3 because nobody owns the lifecycle
- No subject-erasure procedure â€?GDPR Art. 17 would be done by hand under deadline pressure
...
- Plus residual GDPR fine risk on EU exposure: max â‚?0M or 4% of revenue â€?call this $0M expected but tail-risk material

**How this platform fixes it**:
- Lineage end-to-end through dbt DAG + manual trace ([GOVERNANCE.md Â§ Lineage](../governance/GOVERNANCE.md))
- Three-checkpoint quality framework ([GOVERNANCE.md Â§ Quality](../governance/GOVERNANCE.md))
...
## 3. The writing logic â€?how this case was built

This section makes the construction of the case explicit so reviewers can see the reasoning, push back on numbers, and not get sold a black-box ROI. Stakeholders are sceptical of business cases for a reason â€?most are reverse-engineered from a desired conclusion. This one's path is open.

### Step 1 â€?Anchor every pain to a concrete event
...
**Test**: every pain in Â§1 names a specific incident, a specific dollar number, or a specific person's named time. No abstractions.

### Step 2 â€?Annualise the impact
...
- For billing miss: take the worst recent quarter and assume half-rate baseline â†?$2.4M / yr (deliberately conservative; the actual Q4 was $4.8M annualised, but using half makes the case harder to dismiss as cherry-picking).
- For DS time: 12 DS Ã— 8 h / wk Ã— $250 / h Ã— 50 wk â€?multiply, show the arithmetic. Sceptics can debate the 8 h, but the math is transparent.
...
### Step 3 â€?Match each pain to a specific platform capability
...
| Compliance | Lineage + retention + quality | [docs/GOVERNANCE.md](../governance/GOVERNANCE.md) |

**Test**: each pain in Â§1 has a Â§ "How this platform fixes it" with code-level evidence.

### Step 4 â€?Be honest about Year-1 vs. Year-2 reduction

A platform doesn't go to 100% benefit on day 1. Be explicit about ramp:

- Year 1: ~70% reduction â€?MVP is shipped, but adoption is incomplete, automation is partial, customers still use old habits during transition.
- Year 2: ~85% reduction â€?adoption complete, automation production-grade.

Sceptics will probe the ramp. Sandbagging Year 1 (70%, not 100%) is the credible posture.

### Step 5 â€?Separate hard-dollar from strategic enablement

The federal-market enablement (compliance pain) is potentially $40M / year. It would dominate the ROI calculation if included â€?and that's why we don't include it in the payback math. Treat it as a **separate**, **discrete** strategic option that the platform makes available.

**Test**: the payback period (18 months) and 2-year ROI (220%) are computed **without** the federal market in the numerator. The federal opportunity is called out in Â§1 pain 4 but excluded from Â§2.

### Step 6 â€?Show the cost honestly

Capex $2.4M and opex $1.8M / year are not estimates â€?they're sized from the COST-MODEL.md cost breakdown:
...
| Migration consultants (capex only) | $0.5M | â€?|
| Phase 4 hardening (capex) | $0.5M | â€?|
| Ongoing platform-eng FTE (3 Ã— $200K) | â€?| $0.6M |
| Buffer (10%) | $0.0M | (already included) |
| **Total** | **$2.4M** | **$1.9M** rounded â†?$1.8M after Year-2 optimisations |

**Test**: every line traces to a specific cloud SKU or FTE rate. No "miscellaneous" line.

### Step 7 â€?Build a credible NPV
...
Plus terminal value at 5% perpetuity beyond year 3:
  = (2.7 Ã— 1.05 / (0.10 - 0.05)) / 1.331
  = 56.7 / 1.331
  = ~$42.6M

â†?Full NPV: ~$45M â€?but this is sensitive to terminal-value
...
### Step 8 â€?Identify what would invalidate the case

Honesty about failure modes makes the case more credible, not less.

| Invalidator | Probability | If true: |
|---|---|---|
| DE productivity savings smaller (DS don't adopt the API) | Medium | Roughly halves the $1.7M; payback slips to ~24 months. Mitigation: invest in DS onboarding from week 1 (R8 in [REQUIREMENTS-ANALYSIS.md Â§ Risks](../analysis/REQUIREMENTS-ANALYSIS.md)) |
...
| Reconciliation error | Â±8% | Â±2% | Â±0.5% | Â±0.5% |
...
1. **The pain is compounding.** Each quarter of manual billing close burns ~$0.6M (annualised $2.4M Ã· 4). At current trajectory we cross break-even on this dimension alone in about 18 months â€?and we still have all the deferred federal-market upside.
2. **The federal opportunity has a 2027 audit-readiness deadline.** SOC 2 Type II takes 12 months of operating evidence. Starting the platform in 2026 means audit-ready by mid-2027 â€?late but viable. Starting in 2027 misses the window entirely.
...
- **Real-time intra-day billing** â€?daily close is the commitment. Intra-day is a Phase 4+ conversation if customers demand it.
- **Multi-cloud at MVP** â€?AWS-first. GCP / Azure are aspirational; the abstraction layer keeps them tractable but not free.
- **AI-powered insights / anomaly detection** â€?the platform serves data, not models. Anomaly detection on top of the marts is a downstream product.
- **Customer-facing dashboards** â€?REST API only. Customer UI is a separate product roadmap item.
- **Inference prompt / completion text storage** â€?out of scope until a PII redaction pipeline is in place.
...
| **Headcount** | 4 platform engineers + 1 architect (this document's author) â€?internal reallocation, no new hires |
| **Timeline** | 6 months MVP â†?12 months GA â†?18 months SOC 2 Type II |
| **First gate** | End of month 3: end-to-end Bronze â†?Silver â†?Gold path in staging, quality gate active |
...
[REQUIREMENTS-ANALYSIS.md](../analysis/REQUIREMENTS-ANALYSIS.md) for the detailed risk register;           
