...
> Phase 1 deliverable for project-304. Reverses the requirements out of the implementation, validates them against scenarios, traces them to architecture components, and inventories assumptions + risks. Read alongside [304requirements.md](../304requirements.md) (the requirements spec) and [BUSINESS-CASE.md](BUSINESS-CASE.md) (the why).
...
Every requirement in [304requirements.md](../304requirements.md) traces back to one of four business drivers. The chain below makes that mapping explicit — useful when an executive asks "why is FR-X in scope?"
...
**Verdict**: ✅ Designed for. Documented in [README.md § pattern 6](../README.md) and [docs/SECURITY.md § Kafka durability](SECURITY.md).
...
10 risks, scored Impact × Probability on a 1–5 scale. The full table is in [304requirements.md § Risks](../304requirements.md). The summary below highlights what's worth executive attention.
...
The table in [304requirements.md § Traceability](../304requirements.md) maps each requirement to architecture component + code + test. The extended view below adds **the validation evidence** for every Must-Have FR — what specifically demonstrates the requirement is satisfied.
...
- [x] Stakeholders identified (see [STAKEHOLDERS.md](STAKEHOLDERS.md))
...
- [x] Out-of-scope explicitly listed in [304requirements.md § Out of scope](../304requirements.md)
...
**Next**: Move to architecture design — start with [ARCHITECTURE.md](ARCHITECTURE.md), then 5 ADRs in [adr/](adr/).
